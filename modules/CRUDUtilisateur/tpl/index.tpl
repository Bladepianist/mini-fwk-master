	...
	<table class='table table-striped'>
		...
		{foreach $data as $ligne=>$donnees}
			<tr class='table-striped'>
				<td>{$donnees.idUtilisateur}</td>
				<td>{$donnees.nomUtilisateur}</td>
				<td>{$donnees.prenomUtilisateur}</td>
				<td>
					<!--voir le détail-->
					<a class='glyphicon glyphicon-search' 
						data-toggle="modal" 
						data-target="#inclusion" 
						href='?module=CRUDUtilisateur&action=detail&id={$donnees.idUtilisateur}&displayModuleInDialog=1'>
					</a> 				
					...
					<a class='glyphicon glyphicon-pencil' 
						data-toggle="modal" 
						data-target="#inclusion"
						href='?module=CRUDUtilisateur&action=modifier&id={$donnees.idUtilisateur}&displayModuleInDialog=1'>
					</a>

					<!--supprimer-->
					<a class='glyphicon glyphicon-remove' title='Supprimer {$donnees.nomUtilisateur} {$donnees.prenomUtilisateur}' 
						href='?module=CRUDUtilisateur&action=supprimer&id={$donnees.idUtilisateur}'></a>				
				</td>
			</tr>
		{foreachelse}	
