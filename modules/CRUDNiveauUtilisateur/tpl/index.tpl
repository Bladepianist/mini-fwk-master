	...
	<table class='table table-striped'>
		...
		{foreach $data as $ligne=>$donnees}
			<tr class='table-striped'>
				<td>{$donnees.idNiveauUtilisateur}</td>
				<td>{$donnees.nomNiveauUtilisateur}</td>
				<td>
					<!--voir le détail-->
					<a class='glyphicon glyphicon-search' 
						data-toggle="modal" 
						data-target="#inclusion" 
						href='?module=CRUDNiveauUtilisateur&action=detail&id={$donnees.idNiveauUtilisateur}&displayModuleInDialog=1'>
					</a> 				
					...
					<a class='glyphicon glyphicon-pencil' 
						data-toggle="modal" 
						data-target="#inclusion"
						href='?module=CRUDNiveauUtilisateur&action=modifier&id={$donnees.idNiveauUtilisateur}&displayModuleInDialog=1'>
					</a>

					<!--supprimer-->
					<a class='glyphicon glyphicon-remove' title='Supprimer {$donnees.nomNiveauUtilisateur}' 
						href='?module=CRUDNiveauUtilisateur&action=supprimer&id={$donnees.idNiveauUtilisateur}'></a>				
				</td>
			</tr>
		{foreachelse}	
