	...
	<table class='table table-striped'>
		...
		{foreach $data as $ligne=>$donnees}
			<tr class='table-striped'>
				<td>{$donnees.idControle}</td>
				<td>{$donnees.typeControle}</td>
				<td>{$donnees.coeffControle}</td>
				<td>
					<!--voir le détail-->
					<a class='glyphicon glyphicon-search' 
						data-toggle="modal" 
						data-target="#inclusion" 
						href='?module=CRUDControle&action=detail&id={$donnees.idControle}&displayModuleInDialog=1'>
					</a> 				
					...
					<a class='glyphicon glyphicon-pencil' 
						data-toggle="modal" 
						data-target="#inclusion"
						href='?module=CRUDControle&action=modifier&id={$donnees.idControle}&displayModuleInDialog=1'>
					</a>

					<!--supprimer-->
					<a class='glyphicon glyphicon-remove' title='Supprimer {$donnees.typeControle}' 
						href='?module=CRUDControle&action=supprimer&id={$donnees.idControle}'></a>				
				</td>
			</tr>
		{foreachelse}	
