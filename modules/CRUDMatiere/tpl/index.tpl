	...
	<table class='table table-striped'>
		...
		{foreach $data as $ligne=>$donnees}
			<tr class='table-striped'>
				<td>{$donnees.idMatiere}</td>
				<td>{$donnees.nomMatiere}</td>
				<td>{$donnees.coeffMatiere}</td>
				<td>
					<!--voir le détail-->
					<a class='glyphicon glyphicon-search' 
						data-toggle="modal" 
						data-target="#inclusion" 
						href='?module=CRUDMatiere&action=detail&id={$donnees.idMatiere}&displayModuleInDialog=1'>
					</a> 				
					...
					<a class='glyphicon glyphicon-pencil' 
						data-toggle="modal" 
						data-target="#inclusion"
						href='?module=CRUDMatiere&action=modifier&id={$donnees.idMatiere}&displayModuleInDialog=1'>
					</a>

					<!--supprimer-->
					<a class='glyphicon glyphicon-remove' title='Supprimer {$donnees.nomMatiere}' 
						href='?module=CRUDMatiere&action=supprimer&id={$donnees.idMatiere}'></a>				
				</td>
			</tr>
		{foreachelse}	
