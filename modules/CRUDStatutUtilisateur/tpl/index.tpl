	...
	<table class='table table-striped'>
		...
		{foreach $data as $ligne=>$donnees}
			<tr class='table-striped'>
				<td>{$donnees.idStatutUtilisateur}</td>
				<td>{$donnees.nomStatutUtilisateur}</td>
				<td>
					<!--voir le détail-->
					<a class='glyphicon glyphicon-search' 
						data-toggle="modal" 
						data-target="#inclusion" 
						href='?module=CRUDStatutUtilisateur&action=detail&id={$donnees.idStatutUtilisateur}&displayModuleInDialog=1'>
					</a> 				
					...
					<a class='glyphicon glyphicon-pencil' 
						data-toggle="modal" 
						data-target="#inclusion"
						href='?module=CRUDStatutUtilisateur&action=modifier&id={$donnees.idStatutUtilisateur}&displayModuleInDialog=1'>
					</a>

					<!--supprimer-->
					<a class='glyphicon glyphicon-remove' title='Supprimer {$donnees.nomStatutUtilisateur}' 
						href='?module=CRUDStatutUtilisateur&action=supprimer&id={$donnees.idStatutUtilisateur}'></a>				
				</td>
			</tr>
		{foreachelse}	
