	...
	<table class='table table-striped'>
		...
		{foreach $data as $ligne=>$donnees}
			<tr class='table-striped'>
				<td>{$donnees.idGroupeUtilisateur}</td>
				<td>{$donnees.nomGroupeUtilisateur}</td>
				<td>
					<!--voir le détail-->
					<a class='glyphicon glyphicon-search' 
						data-toggle="modal" 
						data-target="#inclusion" 
						href='?module=CRUDGroupeUtilisateur&action=detail&id={$donnees.idGroupeUtilisateur}&displayModuleInDialog=1'>
					</a> 				
					...
					<a class='glyphicon glyphicon-pencil' 
						data-toggle="modal" 
						data-target="#inclusion"
						href='?module=CRUDGroupeUtilisateur&action=modifier&id={$donnees.idGroupeUtilisateur}&displayModuleInDialog=1'>
					</a>

					<!--supprimer-->
					<a class='glyphicon glyphicon-remove' title='Supprimer {$donnees.nomGroupeUtilisateur}' 
						href='?module=CRUDGroupeUtilisateur&action=supprimer&id={$donnees.idGroupeUtilisateur}'></a>				
				</td>
			</tr>
		{foreachelse}	
