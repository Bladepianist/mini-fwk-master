<?php
class CRUDDepartement extends Module{
	...
	public function action_index(){
		...
		$data = DepartementManager::listerDepartement();
	...
	public function action_modifier(){
		...
		$data = DepartementManager::chercherParID($id);
		
		$this->set_title("Modifier"); //OK
		$f=new Form("?module=CRUDDepartement&action=valide&mod=modifier&id=".$id,"form update"); //OK
		$f->add_text("nomDepartement","nomDepartement","Département")-> set_required(); //OK
		$f->add_submit("Valider","bntval")->set_value('Modifier');		
		
		// Remet les valeurs d'origine (pré-remplissage)
		$f->populate(array("nomDepartement" => $data->nomDepartement));
	...
	public function action_valide(){
		...
		if($this->req->nomDepartement == ''){
			$this->site->ajouter_message('Veuillez saisir un nom valide',ALERTE);			
			$err=true;
			$form->nomDepartement->set_error(true);
			$form->nomDepartement->set_error_message("champs vide !");
		}
		...
		//tous les tests ont été validés
			...
			//création d'une instance de Departement
			$m=new Departement();
			$m->nomDepartement=$this->req->nomDepartement;
			
			//enregistrement (insertion) dans la base
                        if(isset($mod) && $mod== "modifier") {
                            
                            $mb->modifierDepartement($m, $id);
                            //passe un message pour la page suivante
                            $this->site->ajouter_message('Le département a bien été modifié');
                            //redirige vers le module par défaut
                            $this->site->redirect('CRUDDepartement');
                        }
                        elseif(isset($mod) && $mod=="ajouter"){		
                            
                            $mb->creerDepartement($m);
                            //passe un message pour la page suivante
                            $this->site->ajouter_message('Le département a bien été créé');
                            //redirige vers le module par défaut
                            $this->site->redirect('CRUDDepartement');
                        }
		}
	...
	public function action_supprimer(){
		...
		$User = new DepartementManager();
		
		$User->supprimerDepartement($id);
		$this->site->ajouter_message('Le département a bien été supprimé');
		$this->site->redirect('CRUDDepartement');
	...
	public function action_ajouter(){
		...
		$f=new Form("?module=CRUDDepartement&action=valide&mod=ajouter","form_ajouterUser"); //OK
		$f->add_text("nomDepartement","nomDepartement","Nom")-> set_required(); //OK
	...
	public function action_detail(){
		...
		$this->tpl->assign("nomDepartement",$nomDepartement);
		$this->tpl->assign("prenomDepartement",$prenomDepartement);	*/
	}
